...
- **AC-AGENTS-DYNAMIC-AGENT-ROUTING-001.5:** For ordinary dynamic profiles, the profile name is the only capability label. Users can create profiles such as Frontier, Balanced, Economy, Review, or Security Review. Kandev assigns no semantics to those names. An explicitly enabled OmniRoute policy profile may carry the route classes defined by `REQ-AGENTS-OMNIROUTE-POLICY-001`; those classes do not infer provider or model capabilities.
